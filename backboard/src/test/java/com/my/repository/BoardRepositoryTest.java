...
public class BoardRepositoryTest {
  ...
  public void testSelectByWord() throws FindException {
    int expectesSize = 2; // 총 검색어 "1번"의 총행수는 3행이다. 1페이지별 2건씩 보면서 2페이지의 행수를 예상
    String word = "1번";
    int currentPage = 1;
    int cntPerPage = 2;
    List<Board> list = repository.selectByWord(word, currentPage, cntPerPage);
    assertEquals(expectesSize, list.size());
  }
  ...
  @Test
  public void testUpdate() throws FindException, ModifyException {
    // 조회수 1증가 테스트
    int boardNo = 1; // 1번 글
    ...
    Board b2 = repository.selectByBoardNo(boardNo); // 조회수 증가후의 글
