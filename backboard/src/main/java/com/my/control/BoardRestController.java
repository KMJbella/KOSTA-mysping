...
@RestController
@RequestMapping("board/*") // http://localhost:8888/backboard/board/list
public class BoardRestController {
  ...
  // GET /backboard/board/search/검색어/페이지번호
  // GET /backboard/board/search/검색어
  // GET /backboard/board/search/페이지번호 ????
  // GET /backboard/board/search/페이지번호 ????
  ...
  // POST /backboard/board/write/글제목/글내용 - 파일업로드를 하려면 반드시 폼데이터가 필요.
  ...
  public ResponseEntity<?> write(@RequestPart(required = false) List<MultipartFile> letterFiles,
    ...
    logger.info("파일크기 : imageFile.getSize()=" + imageFile.getSize()
        + ", 업로드된 파일 이름 : imageFile.getOriginalFileName()=" + imageFile.getOriginalFilename());
  ...
  public ResponseEntity<Object> modify(@PathVariable int boardNo, @RequestBody Board b) {
    try {
  ...
  public ResponseEntity<String> reply(@PathVariable int boardParentNo, @RequestBody Board b) {
    // String loginedId = (String)session.getAttribute("loginInfo");
    // ---로그인대신할 샘플데이터--
    String loginedId = "id1";
    b.setBoardId(loginedId);
    b.setBoardParentNo(boardParentNo);
    if (b.getBoardTitle() == null || b.getBoardTitle().equals("") || b.getBoardContent() == null
        || b.getBoardContent().equals("")) {
